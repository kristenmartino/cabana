        ...
        {!sent ? (
          ...
          <div className="rounded-3xl bg-card p-6 text-center shadow-card">
            ...
            <div className="mt-6 text-xs text-muted-foreground">
              For demo purposes: <Link href="/" className="font-semibold text-lagoon">continue to portal →</Link>
            </div>
          </div>
        )}

        <p className="mt-8 text-center text-sm text-muted-foreground">
          Not a member yet? Call {MOCK.supportName} at{" "}
          <a href={`tel:${MOCK.supportPhone.replace(/[^\d]/g, "")}`} className="font-semibold text-deepwater underline-offset-4 hover:underline">
            {MOCK.supportPhone}
          </a>
          .
        </p>
